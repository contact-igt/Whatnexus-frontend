...
interface DataSourceProps {
    ...
    handleDragLeave: any;
    handleDrop: any;
    handlePermanentDeleteClick: any;
    handleRestore: any;
...
export const DataSource = ({ isDarkMode, setSelectedItem, isDragging, uploadedData, setUploadedData, setIsDragging, handleDragEnter, handleDragOver, handleDragLeave, handleDrop, handleUploadFile, handleDeleteClick, handlePermanentDeleteClick, handleRestore, handleEdit, handleView, uploading }: DataSourceProps) => {
    const fileInputRef = useRef<HTMLInputElement>(null);
    const { user } = useAuth();
    const { data: knowledgeData, isLoading: isKnowledgeLoading, isError } = useGetKnowledgesQuery();
    const { data: deletedKnowledgesData, isLoading: isDeletedKnowledgeLoading } = useDeletedKnowledgeList();
            ...
            <GlassCard isDarkMode={isDarkMode} className="p-6">
                ...
                <div className="space-y-3">
                    {isKnowledgeLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-[200px] text-center">
                            <Database className={cn("mb-3", isDarkMode ? 'text-white/20' : 'text-slate-300')} size={40} />
                            <p className={cn("text-sm", isDarkMode ? 'text-white/40' : 'text-slate-400')}>
            ...
            <GlassCard isDarkMode={isDarkMode} className="p-6">
                ...
                <div className="space-y-3">
                    {isDeletedKnowledgeLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-[200px] text-center">
                            <Trash2 className={cn("mb-3", isDarkMode ? 'text-white/20' : 'text-slate-300')} size={40} />
                            <p className={cn("text-sm", isDarkMode ? 'text-white/40' : 'text-slate-400')}>
                                No deleted sources found
                            </p>
                            <p className={cn("text-xs mt-1", isDarkMode ? 'text-white/30' : 'text-slate-400')}>
