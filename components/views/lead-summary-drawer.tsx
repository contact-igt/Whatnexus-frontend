...
export const LeadSummaryDrawer = ({ isOpen, onClose, lead, isDarkMode }: LeadSummaryDrawerProps) => {
    ...
    return (
        <Drawer
            ...
            title="Neural Lead Summary"
            description={`Generating insights for ${lead?.name}`}
            className="sm:max-w-xl"
        >
            <div className="flex flex-col font-sans">
                {/* Date Picker Toggle / Display */}
                <div className={cn("px-1 py-3 flex items-center justify-between border-b transition-colors mb-4 font-sans", isDarkMode ? "border-white/5" : "border-slate-100")}>
                ...
                {showDatePicker && (
                    <div className={cn("border rounded-xl mb-6 overflow-hidden animate-in slide-in-from-top-2", isDarkMode ? "border-white/5 bg-black/20" : "border-slate-100 bg-slate-50/50")}>
                        <div className="flex flex-col md:flex-row h-auto md:h-[320px]">
                            {/* Sidebar Presets */}
                            <div className="w-full md:w-40 border-b md:border-b-0 md:border-r p-3 pb-4 flex flex-col gap-1 overflow-y-auto">
                                {presets.map((preset) => (
                                    <button
                                        key={preset.label}
                ...
                <div className={cn("p-6 rounded-xl min-h-[200px] flex-1 overflow-y-auto relative border", isDarkMode ? "bg-white/5 border-white/5" : "bg-slate-50 border-slate-100")}>
                    {isLoading ? (
