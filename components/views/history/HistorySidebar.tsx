...
interface HistorySidebarProps {
    ...
    chatFilter: 'all' | 'read' | 'unread';
    setChatFilter: (filter: 'all' | 'read' | 'unread') => void;
            ...
            <div className="p-2 space-y-2">
                ...
                <div className="flex items-center gap-1 overflow-x-auto no-scrollbar pb-1 px-1">
                    {['All', 'Read', 'Unread'].map(f => {
                        const filterId = f.toLowerCase() as 'all' | 'read' | 'unread';
                        const isActive = chatFilter === filterId;
                        return (
                            <button
                                key={f}
                                onClick={() => setChatFilter(filterId)}
                                className={cn(
